...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.kampanye.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.kampanyes.update", [$kampanye->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="nama_kampanye">{{ trans('cruds.kampanye.fields.nama_kampanye') }}</label>
                <input class="form-control {{ $errors->has('nama_kampanye') ? 'is-invalid' : '' }}" type="text" name="nama_kampanye" id="nama_kampanye" value="{{ old('nama_kampanye', $kampanye->nama_kampanye) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" type="text" name="slug" id="slug" value="{{ old('slug', $kampanye->slug) }}" required>
                @if($errors->has('slug'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total') ? 'is-invalid' : '' }}" type="number" name="total" id="total" value="{{ old('total', $kampanye->total) }}" step="0.01" required>
                @if($errors->has('total'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('image') ? 'is-invalid' : '' }}" type="text" name="image" id="image" value="{{ old('image', $kampanye->image) }}">
                @if($errors->has('image'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('deskripsi') ? 'is-invalid' : '' }}" name="deskripsi" id="deskripsi">{{ old('deskripsi', $kampanye->deskripsi) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('lokasi') ? 'is-invalid' : '' }}" type="text" name="lokasi" id="lokasi" value="{{ old('lokasi', $kampanye->lokasi) }}" required>
