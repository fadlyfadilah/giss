...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.donatur.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.donaturs.update", [$donatur->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $donatur->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nama') ? 'is-invalid' : '' }}" type="text" name="nama" id="nama" value="{{ old('nama', $donatur->nama) }}" required>
                @if($errors->has('nama'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="text" name="email" id="email" value="{{ old('email', $donatur->email) }}" required>
                @if($errors->has('email'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nohp') ? 'is-invalid' : '' }}" type="number" name="nohp" id="nohp" value="{{ old('nohp', $donatur->nohp) }}" step="1" required>
                @if($errors->has('nohp'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('jumlah') ? 'is-invalid' : '' }}" type="number" name="jumlah" id="jumlah" value="{{ old('jumlah', $donatur->jumlah) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('pesan') ? 'is-invalid' : '' }}" name="pesan" id="pesan">{{ old('pesan', $donatur->pesan) }}</textarea>
                @if($errors->has('pesan'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kampanye') ? 'is-invalid' : '' }}" name="kampanye_id" id="kampanye_id">
                    @foreach($kampanyes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('kampanye_id') ? old('kampanye_id') : $donatur->kampanye->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
